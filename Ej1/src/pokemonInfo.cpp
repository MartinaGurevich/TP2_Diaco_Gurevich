...
void PokemonInfo::deserializarInfo(ifstream& in) {
    //deserializo el tipo
    size_t tipoSize = tipo.size();
    in.read(reinterpret_cast<const char*>(&tipoSize), sizeof(tipoSize));
    string tipo_ = string(tipoSize, '\0')
    in.read(&(tipo_)[0], tipoSize); 

    //deserializo la descripcion
    size_t descripcionSize = descripcion.size();
    in.read(reinterpret_cast<const char*>(&descripcionSize), sizeof(descripcionSize));
    ...
    for(const auto& ataque_daño: ataques){
        //deserializo key
        size_t keySize = ataque_daño.first.size();
        in.read(reinterpret_cast<const char*>(&keySize), sizeof(keySize));
        string key_ = string(keySize, '\0')
        in.read(&(key_)[0], keySize); 

        //deserializo value
        in.read(reinterpret_cast<const char*>(&ataque_daño.second), sizeof(ataque_daño.second));
